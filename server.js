...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    else cb(new Error('Only image, audio and video files allowed'));
  }
});
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const new_user = await run('INSERT INTO users (username, password_hash, last_seen) VALUES (?, ?, CURRENT_TIMESTAMP)', [username, hash]);
...
app.post('/api/chats/:chatId/kick', async (req, res) => {
  ...
  try {
    ...
    if (Number(chat.creator_id) != Number(userId)) return res.status(403).json({ error: 'Only creator can kick' });
